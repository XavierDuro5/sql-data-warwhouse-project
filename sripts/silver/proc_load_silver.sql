...
BEGIN
	...
	BEGIN TRY
		...
		CASE WHEN UPPER(TRIM(cst_material_statu)) = 'S' THEN 'Single'
			 WHEN UPPER(TRIM(cst_material_statu)) = 'M' THEN 'Maried'
			 ELSE 'n/a'
		END cst_material_statu,
		...
		CAST(LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE) AS prd_end_dt -- end date one day before the next satart date
		...
		SELECT
			...
			END AS sls_due_dt,
			CASE WHEN sls_dales IS NULL OR sls_dales <= 0 OR sls_dales != sls_quantity * ABS(sls_price)
				THEN sls_quantity * ABS(sls_price)
			ELSE sls_dales
			END AS sls_dales,
			sls_quantity,
			CASE WHEN sls_price IS NULL OR sls_price <= 0
				THEN sls_dales / NULLIF(sls_quantity, 0)
			ELSE sls_price
			END AS sls_price
